...
```shell
mysqldump -u root -p<your_mysql_root_password> <database_name> > /tmp/<backup_file_name>.sql
```

- Replace `<your_mysql_root_password>` with the actual password.
...
```
docker exec -it <mysql_container_name_or_id> mysqldump -u root -p'myrootpassword' <database_name> > /backups/<backup_file_name>.sql
```
